...
print('******')
hf_dmet = ssdmet.SSDMET(mf, title=title)
hf_dmet.imp_idx = mol.search_ao_label('Co *')
hf_dmet.build(chk_fname_load=title + '_dmet_chk.h5')
es_mf = hf_dmet.ROHF(run_mf=True)
ncas, nelec, es_mo = hf_dmet.avas(['Co 3d', 'Co 4d'], minao='def2tzvp', threshold=0.5)
es_cas = sacasscf_mixer.sacasscf_mixer(es_mf, ncas, nelec, statelis=[0, 40, 0, 10])
es_cas.kernel(es_mo)
...
print('******')
cas_dmet = ssdmet.SSDMET(mycas, title=title+'_cas')
cas_dmet.imp_idx = mol.search_ao_label('Co *')
cas_dmet.build()
es_mf = cas_dmet.ROHF(run_mf=False)
